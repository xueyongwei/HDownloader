...
- (NSURLSessionDownloadTask *)_downloadTaskWithOriginResumeData:(NSData *)resumeData withDownloadModel:(DownloadModel *)topWaitingModel
{
    ...
    NSURLSessionDownloadTask *task = [self.urlSession downloadTaskWithResumeData:cData progress:^(NSProgress * _Nonnull downloadProgress) {
        topWaitingModel.videoSize = downloadProgress.totalUnitCount;
        ...
        DDLogVerbose(@"videoSize = %ld",topWaitingModel.videoSize);
        [DownloadManager postNotification:DownloadingUpdateNotification andObject:topWaitingModel];
        
    } destination:^NSURL * _Nonnull(NSURL * _Nonnull targetPath, NSURLResponse * _Nonnull response) {
        return [NSURL fileURLWithPath:[DownloadManager getMP4LocalUrlWithVideoName:topWaitingModel.name]];
        ...
        if (task.currentRequest == nil)
        {
            ...
            if (currentRequest)
            {
                @try {
                    ...
                    task = [self.urlSession downloadTaskWithRequest:rq progress:^(NSProgress * _Nonnull downloadProgress) {
                        topWaitingModel.videoSize = downloadProgress.totalUnitCount;
                        topWaitingModel.downloadPercent = downloadProgress.completedUnitCount / (downloadProgress.totalUnitCount * 1.0);
                        DDLogVerbose(@"videoSize = %ld",topWaitingModel.videoSize);
                        [self.downloadCacher updateDownloadModel:topWaitingModel];
                        ...
                        NSURL *fileURL = [NSURL fileURLWithPath:[DownloadManager getMP4LocalUrlWithVideoName:topWaitingModel.name]];
                        [self.downloadCacher updateDownloadModel:topWaitingModel];
                        topWaitingModel.name = fileURL.lastPathComponent;
                        return fileURL;
